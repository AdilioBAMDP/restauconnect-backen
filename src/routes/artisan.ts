...
router.get('/inventory', authenticateToken, async (req, res) => {
  try {
    // Filtrage par artisan connectÃƒÂ© (supplierId = user._id ou user.id ou user.userId)
    const userAny = req.user as any;
    const artisanId = userAny?._id || userAny?.id || userAny?.userId;
    if (!artisanId) {
      return res.status(401).json({ success: false, error: 'Utilisateur non authentifiÃƒÂ©' });
    }
    ...
    const mapped = products.map((p: any) => ({
      ...
      category: p.category || 'Non catÃƒÂ©gorisÃƒÂ©',
    ...
    console.error('Ã¢ÂÅ’ Erreur /artisan/inventory:', error);
    res.status(500).json({ 
      success: false, 
