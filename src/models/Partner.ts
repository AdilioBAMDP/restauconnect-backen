...
export interface IPartner extends Document {
  userId?: mongoose.Types.ObjectId; // Référence vers User si le partenaire est inscrit
  name: string;
...
/**
 * Schéma MongoDB pour les Partenaires
 */
const PartnerSchema: Schema = new Schema(
  {
    ...
    specialty: {
      ...
      trim: true,
      maxlength: [200, 'La spécialité ne peut pas dépasser 200 caractères']
    },
    location: {
      type: String,
    ...
    skills: {
      ...
      validate: {
        ...
        message: 'Un partenaire ne peut pas avoir plus de 20 compétences'
      }
    },
    experience: {
      type: String,
...
// Méthode pour incrémenter les demandes de contact
PartnerSchema.methods.incrementContactRequests = async function() {
  this.contactRequests += 1;
  return await this.save();
};

// Méthode statique pour obtenir les partenaires par rôle (avec exclusion)
...
// Méthode statique pour obtenir les stats par rôle
